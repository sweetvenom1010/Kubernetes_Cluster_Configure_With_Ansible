...
options:
    ...
    feature_gates:
        ...
        options:
            ...
            PublicKeysECDSA:
                ...
                default: false
    force:
        description: Reset kubernetes before initialize if by chance system is already initialized
        required: false
        type: bool
        default: False
    ignore_preflight_errors:
        description: List of Preflight checks if any error occurs in that checks
        required: false
        type: list
    image_repository:
    ...
    kubernetes_version:
        ...
        default: stable-1
    node_name:
        description: Specify Node Name 
        type: str
        required: false
    pod_network_cidr:
        description: Specify Pod Network CIDR block
        required: true
        type:str
    service_cidr:
        description: Specify Service CIDR block
        required: false
        type: str
    ...
    token:
        ...
        type: str
    token_ttl:
        description: Token Validation Time Interval
        required: false
        type: str
        default: 24h0m0s
    upload_certs:
...
def run_module():
    ...
    module_args = dict(
        ...
        feature_gates=dict(type='dict',options=dict(
            IPv6DualStack=dict(type='bool',default=True,choices=[True,False]),
            PublicKeysECDSA=dict(type='bool',default=True,choices=[True,False])
            )),
    ...
    if force == True:
        ...
        if getstatusoutput("kubectl cluster-info")[0] != 0 or getstatusoutput("kubectl cluster-info --kubeconfig /etc/kubernetes/admin.conf")[0] != 0:
            status , output = getstatusoutput(cmd)
            if status==0:
                result["changed"] = True
            else:
